...
10. Merge ONLY when ALL required checks green. **`dev` and `main` are mechanically protected as of 2026-05-12** — `dev` requires the 5 PR checks (ESLint, gitleaks, commitlint, danger, fallow) + `strict` (up-to-date-with-base), `main` requires `ci.yml` + `strict` — so a non-green / out-of-date PR is blocked at the API regardless. Admin override is reserved for `dev → main` rollup PRs only (per `feedback_rollup_pr_admin_squash.md`); feature → dev PRs always wait for green CI. Doc-only PRs with green CI + no actionable CR may merge without further wait. (No merge queue — `strict` covers the race; see `docs/ci-cd-pipeline-spec.md` §4.4.)
